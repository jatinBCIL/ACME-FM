...
public class PrintDirect
{
    ...
    public static Boolean Print_DispenseRawLabel(string printerName, string strBarcode, string strMatCode, string strMatDescription,
                                                 ...
                                                 string strBatchSize, string strARNo, string strContainerNo, string strLotNo,string strip, string port,string strPrintFlag , string LeastCount)
    {
        IntPtr lhPrinter = new IntPtr();
        DOCINFO Doc = new DOCINFO();
        int pcWritten = 0;
        ...
        try
        {
            ...
            //}
            string[] dd = strARNo.Split('/');
            strMatType = (dd[0] == "ST" ? dd[1].Substring(1, 2) : dd[0].Substring(1, 2));
            string block = (dd[0] == "ST" ? dd[1].Substring(0, 1) : dd[0].Substring(0, 1));
            if (strMatType == "RM")
            {
                strFileData = ReadPrn("Dispensing-1.prn");
                strPrnFile = "Dispensing-1";
            }
            else if (strMatType == "PM")
            {
                strFileData = ReadPrn("Dispensing-2.prn");
            ...
            {
                strFileData = strFileData.Replace("{GrossWt}", Math.Round(dGrosswt ,int.Parse(LeastCount)) + " " + strUom);
            ...
            {
                strFileData = strFileData.Replace("{TareWt}", Math.Round(dTarewt, int.Parse(LeastCount)) + " " + strUom);
            }

            strFileData = strFileData.Replace("{NetWt}", Math.Round(dNetwt, int.Parse(LeastCount)) + " " + strUom);
            strFileData = strFileData.Replace("{BARCODE}", strBarcode.Trim());
        ...
        try
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
             ...
             //   strFileData = ReadPrn("MateialRecieptSuLabel.prn");
                ...
                {
                    strFileData = ReadPrn("MateialRecieptSuLabel_PM.prn");
                    strPrnFile = "MateialRecieptSuLabel_PM";
                }
                else if (dt.Rows[i]["MaterialType"].ToString().ToUpper() == "RM")
                {
                    strFileData = ReadPrn("MateialRecieptSuLabel_RM.prn");
                    strPrnFile = "MateialRecieptSuLabel_RM";
                }
                //CreateLog("PRN NAME" + strPrnFile);




                if (dt.Rows[i]["GRNNo"].ToString().Substring(3, 1).Trim() == "H")
                {
                    strFormatNo = objData.ExecuteScalarString(objData.strLocal, "SELECT FORMAT_NO FROM [TBL_PRINT_FORMAT] WHERE BLOCK = 'HB' AND PRN_FILE_NAME = '" + strPrnFile + "'");
                }
                else if (dt.Rows[i]["GRNNo"].ToString().Substring(3, 1).Trim() == "G")
                {
                ...
                strFileData = strFileData.Replace("{BARCODE}", dt.Rows[i]["BarcodeNo"].ToString());
                strFileData = strFileData.Replace("{Item Code}", dt.Rows[i]["MaterialCode"].ToString());
                strFileData = strFileData.Replace("{MaterialCode}", dt.Rows[i]["MaterialCode"].ToString());
                strFileData = strFileData.Replace("{Item Name}", dt.Rows[i]["MaterialName"].ToString());
                ...
                //else
                //{
                    string mfgDate =  (DateTime.Parse(dt.Rows[i]["MFGDate"].ToString()) < DateTime.Parse("01-Jan-1920") ? "NA" : DateTime.Parse(dt.Rows[i]["MFGDate"].ToString()).ToString("MM/yy").Replace('-', '/'));
                    strFileData = strFileData.Replace("{MFG Date}", mfgDate);
                    if (dt.Rows[0]["V_Retest"].ToString() == "0")
                    {
                        string expDate = (DateTime.Parse(dt.Rows[i]["EXPDate"].ToString()) < DateTime.Parse("01-Jan-1920") ? "NA" : DateTime.Parse(dt.Rows[i]["EXPDate"].ToString()).ToString("MM/yy").Replace('-', '/'));
                        strFileData = strFileData.Replace("{Exp Date}",expDate);
                        strFileData = strFileData.Replace("{ReDate}", "NA");
                    }
                    else
                    {
                ...
                CreateLog(strFileData.ToString());
    ...
    public static void CreateLog(string messege)
    {
        var fileName = "D:\\DispensingReprint.txt";
