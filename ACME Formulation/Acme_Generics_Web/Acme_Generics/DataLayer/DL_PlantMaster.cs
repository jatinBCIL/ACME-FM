...
namespace DataLayer
{
    public class DL_PlantMaster
    {
            ...
            DL_getPlantCode_Desc()
        {
            ...
            try
            {
                str = "SELECT Distinct [NAME]  FROM TBLMASTER_CONSIGNEE";
        ...
        public DataTable DL_getPlantDtl(string strWhere)
        {
            DataSet ds = new DataSet();
            SqlDataLayer objData = new SqlDataLayer();
            ds = objData.ExecuteDataset(objData.strLocal, "SELECT *  FROM TBLMASTER_CONSIGNEE" + strWhere.Trim());
            return ds.Tables[0];
        ...
        public DataTable DL_Get_Plant_Type()
        {
            ...
            try
            {
                str = "SELECT Distinct PLANT_ID+'-'+PLANT_NAME as PLANTCODE FROM TBLMASTER_PLANT";
                ds = objSql.ExecuteDataset(objSql.strLocal, str);
                if (ds.Tables[0].Rows.Count > 0)
                {
                    dt = ds.Tables[0];
