    ...
    public class DL_GrnPrinting
    {
        ...
        public DataTable DL_GetARDocumenDetails(string strDocumentNo, string strPrintType)
        {
            ...
            try
            {
                if (strPrintType.Trim() == "PRINT")
                {
                    return objSql.ExecuteDataset(objSql.strErp, "SELECT [GRN No_],[GRN Item No_],[Inward No_],[GRN Date], [Material Code],[Material Name], [Material Type],[Batch No_],cast([MFG Date] as date)[MFG Date],cast([EXP Date] as date)[EXP Date],[No_ Of Container],cast([Total Qty_] as numeric(18,3))[Total Qty_],[Supplier Name],[Manufacture Name],[Batch No_],[Storage Condition],[AR NO] FROM [dbo].[Acme Formulations Pvt Ltd$GRN Detail] WHERE [AR NO]='" + strDocumentNo + "' AND ISNULL(Status,0)=0").Tables[0];
                }
                else
                {
        ...
        public int DL_SaveGRNErpStock(string strDocumentNo,string strcont, string strArNo,double tQty)
        {
            ...
            try
            {
                return objSql.ExecuteNonQuery(objSql.strErp, "UPDATE [Acme Formulations Pvt Ltd$GRN Detail] SET [Total Qty_]=" + tQty + " ,[No_ Of Container]=" + int.Parse(strcont) + "  where [GRN No_] = '" + strDocumentNo + "' and [AR NO] = '" + strArNo + "'");
            }
            catch (Exception ex)
            {
                throw new Exception(ex.ToString());
        ...
        public int DL_SaveGRNERPPrint(string strDocumentNo, string strMatcode, string strInwardno)
        {
            ...
            try
            {
                return objSql.ExecuteNonQuery(objSql.strErp, "UPDATE [Acme Formulations Pvt Ltd$GRN Detail] SET [Status]=1 WHERE [GRN No_]='" + strDocumentNo + "' AND [GRN Item No_]='" + strMatcode + "' AND [Material Code]='" + strMatcode + "' and [Inward No_]='" + strInwardno + "'");
            }
            catch (Exception ex)
            {
                throw new Exception(ex.ToString());
        ...
        public DataTable DL_GetDocumentNo(string strDocumentNo, string strPrintType)
        {
            ...
            try
            {
                if (strPrintType.Trim() == "PRINT")
                {
                    dt= objSql.ExecuteDataset(objSql.strErp, "SELECT [GRN No_][GRNNo] FROM [dbo].[Acme Formulations Pvt Ltd$GRN Detail] WITH (NOLOCK) WHERE [GRN No_] LIKE '%" + strDocumentNo + "' AND ISNULL(Status,0)=0").Tables[0];
        ...
        public DataTable DL_GetARDocumentNo(string strDocumentNo, string strPrintType)
        {
            ...
            try
            {
                if (strPrintType.Trim() == "PRINT")
                {
                    dt = objSql.ExecuteDataset(objSql.strErp, "SELECT [AR NO][ARNo] FROM [dbo].[Acme Formulations Pvt Ltd$GRN Detail] WITH (NOLOCK) WHERE [AR NO] LIKE '%" + strDocumentNo + "' AND ISNULL(Status,0)=0").Tables[0];
