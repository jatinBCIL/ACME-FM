...
namespace BusinessLayer
{
    public class BL_Dispensing
    {
        ...
        public string BL_ValidatePicking(string strPickListNo, string strMaterial, string strBatchNo, string strBarcodeNo, string strType, string strUser, string strPlant)
        {

            DL_PickList objDL = new DL_PickList();
            try
            {
                return objDL.DL_ValidatePicking(strPickListNo, strMaterial,"0", strBatchNo, strBarcodeNo, strType,0.00, strUser, strPlant);
            }
            catch (Exception ex)
            {
                throw new Exception(ex.ToString());
