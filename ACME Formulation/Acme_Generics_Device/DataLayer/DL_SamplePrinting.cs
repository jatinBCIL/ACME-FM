...
namespace DataLayer
{
    public class DL_SamplePrinting
    {
        ...
        public DataTable DL_GetDisplayDetails(string strBarcode, string strPlant)
        {
            ...
            try
            {
                return objSql.ExecuteDataset(objSql.strLocal, "Select * from [VW_StatusDisplay] B left outer Join [dbo].[tblGrn_Trans] A ON A.BarcodeNo = B.BarcodeNo WHERE A.[BarcodeNo] =  '" + strBarcode + "' and A.[PLANTCODE] ='" + strPlant + "'").Tables[0];
